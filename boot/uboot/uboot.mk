...
define UBOOT_CONFIGURE_CMDS
	$(if $(BR2_TARGET_UBOOT_WATCHDOG),
		$(SED) '/CONFIG_MACH_TYPE/a #define CONFIG_HW_WATCHDOG' \
		$(@D)/include/configs/$(shell awk '$$7 == "$(UBOOT_BOARD_NAME)" { print $$6 }' $(@D)/boards.cfg).h)
	$(if $(BR2_PACKAGE_SK_DDR528),
		find $(@D)/board/starterkit/ -type f -name *.cfg -exec sed -i '/0x020C4018/d' {} \;)
	$(SED) '0,/CONFIG_SK_VIDEO/s/^.*CONFIG_SK_VIDEO.*$$/#define CONFIG_SK_VIDEO "$(call qstrip,$(BR2_TARGET_UBOOT_FB0) $(BR2_TARGET_UBOOT_FB1) $(BR2_TARGET_UBOOT_CUSTOM_BOOT_CMDLINE))"/' \
		$(@D)/include/configs/$(shell awk '$$7 == "$(UBOOT_BOARD_NAME)" { print $$6 }' $(@D)/boards.cfg).h
	$(SED) '0,/CONFIG_BOOTDELAY/s/^.*CONFIG_BOOTDELAY.*$$/#define CONFIG_BOOTDELAY $(BR2_TARGET_UBOOT_BOOTDELAY)/' \
		$(@D)/include/configs/$(shell awk '$$7 == "$(UBOOT_BOARD_NAME)" { print $$6 }' $(@D)/boards.cfg).h
...
define UBOOT_INSTALL_NAND_IMAGE
	$(TARGET_CONFIGURE_OPTS) $(UBOOT_CONFIGURE_OPTS) 	\
		$(MAKE) -C $(@D) $(UBOOT_MAKE_OPTS)		\
		$(UBOOT_BOARD_NAME)_nandboot_config
	@echo >> $(@D)/include/config.h
	@echo "/* Add a wrapper around the values Buildroot sets. */" >> $(@D)/include/config.h
	@echo "#ifndef __BR2_ADDED_CONFIG_H" >> $(@D)/include/config.h
	@echo "#define __BR2_ADDED_CONFIG_H" >> $(@D)/include/config.h
...
ifeq ($(BR2_PACKAGE_SK_IMX6Q),y)
UBOOT_POST_INSTALL_IMAGES_HOOKS += UBOOT_INSTALL_NAND_IMAGE
endif

ifeq ($(BR2_PACKAGE_SK_IMX6S_OEM),y)
UBOOT_POST_INSTALL_IMAGES_HOOKS += UBOOT_INSTALL_NAND_IMAGE
endif
